...
def process_image_for_highlighted_texts(image_path):
    ...
    for contour in contours:
        ...
        # Perform OCR on the non-white region
        result = reader.readtext(np.array(pil_non_white_region))

        # Extract the detected text from the result and standardize time format
        for detection in result:
            text = detection[1].strip()

            # Standardize the time format to "HH:MM"
            text = text.replace('-', ':').replace('.', ':')  # Fix any format issues

            # Filter out unwanted text and ensure valid time format
            if time_pattern.match(text):  # Only allow valid time in HH:MM format
                time_slots.append(text)

    # Combine extracted time slots
    final_times = "\n".join(time_slots)

    return header_combined, final_times

# Flask route for the API
@app.route("/check")
def check():
    locations = ["Riyadh", "Jeddah", "Al Khobar"]
    #locations = ["Riyadh"]  # For testing purposes, replace with your desired locations
    results = {}
